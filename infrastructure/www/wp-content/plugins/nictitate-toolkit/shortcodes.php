...
function kopa_shortcode_contact_form($atts, $content = null) {
    ...
    $out .= '<label class="required" for="contact_name">' . esc_html__('Name (required):', 'nictitate-toolkit') . '</label>';
    $out .= '<input class="valid" type="text" name="name" id="contact_name" value="">';
    $out .= '</p>';

    $out .= '<p class="input-block clearfix">';
    $out .= '<label class="required" for="contact_email">' . esc_html__('Email (required):', 'nictitate-toolkit') . '</label>';
    $out .= '<input type="email" class="valid" name="email" id="contact_email" value="">';
    $out .= '</p>';

    $out .= '<p class="input-block clearfix">';
    $out .= '<label class="required" for="contact_url">' . esc_html__('Website:', 'nictitate-toolkit') . '</label>';
    $out .= '<input type="url" class="valid" name="url" id="contact_url" value="">';
    $out .= '</p>';

    $out .= '<p class="textarea-block">';
    $out .= '<label class="required" for="contact_message">' . esc_html__('Message (required):', 'nictitate-toolkit') . '</label>';
    $out .= '<textarea id="contact_message" name="message"></textarea>';
    $out .= '</p>';

    $out .= '<p class="comment-button clearfix">';
    $out .= '<input type="submit" id="submit-contact" value="' . esc_html__('Send', 'nictitate-toolkit') . '">';
    $out .= '</p>';
    ...
    if ($atts['address'] || $atts['phone'] || $atts['email']) {
        ...
        $out.= '<h2 class="contact-title">' . esc_html__('Contact Information', 'nictitate-toolkit') . '<span></span></h2>';
        $out.= '<div class="acc-wrapper">
                    <div class="accordion-title">
                        <h3><a href="#">' . esc_html__('Where to find Us?', 'nictitate-toolkit') . '</a></h3>
                        <span>+</span>
                    </div>
...
function kopa_shortcode_pricing_table($atts, $content = '') {
    $atts = shortcode_atts(array(
        'title' => esc_html__('Pricing Table', 'nictitate-toolkit'),
        'columns' => '3'
            ), $atts);
...
function kopa_shortcode_pricing_column($atts, $content = '') {
    $atts = shortcode_atts(array(
        'first_column' => false,
        'special' => false,
        'title' => esc_html__('Pricing Column', 'nictitate-toolkit'),
        'currency_symbol' => '$',
        'price' => 1,
        'plan' => esc_html__('per monthly', 'nictitate-toolkit'),
        'features' => esc_html__('feature 1, feature 2, feature 3, ext...', 'nictitate-toolkit'),
        'button_url' => 'http://kopatheme.com',
        'button_text' => esc_html__('Sign up', 'nictitate-toolkit')
            ), $atts);
